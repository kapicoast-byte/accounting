...
export function newManufacturingLineItem(overrides = {}) {
  return { _id: ++_seq, itemId: '', itemName: '', unit: 'piece', quantity: 1, unitPrice: 0, gstRate: 5, batchNumber: '', productionLogRef: '', ...overrides };
}
...
export default function LineItemEditor({ items, inventoryItems = [], onChange, mode = 'standard' }) {
  ...
  // Manufacturing mode — like retail but adds batch number + production log ref per line
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
              <tr>
                <th className="px-3 py-2 min-w-[180px]">Finished Good</th>
                <th className="px-3 py-2">Unit</th>
                <th className="px-3 py-2 w-20">Qty</th>
                <th className="px-3 py-2 w-28">Rate</th>
                <th className="px-3 py-2 w-24">GST %</th>
                <th className="px-3 py-2 w-32">Batch No.</th>
                <th className="px-3 py-2 w-32">Prod. Log Ref.</th>
                  ...
                  <tr key={line._id}>
                    ...
                    <td className="px-3 py-2">
                      <input type="text" placeholder="e.g. B-001"
                        ...
                        onChange={(e) => update(line._id, { batchNumber: e.target.value })}
                        className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-blue-500" />
                    </td>
                    <td className="px-3 py-2">
                      <input type="text" placeholder="Prod. log ref."
                        value={line.productionLogRef ?? ''}
                        onChange={(e) => update(line._id, { productionLogRef: e.target.value })}
                        className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-blue-500" />
                    </td>
                    <td className="px-3 py-2 text-right text-xs font-medium text-gray-800">
                      {formatCurrency(sub + gst)}
                    </td>
                    <td className="px-3 py-2">
                      <button type="button" onClick={() => remove(line._id)} disabled={items.length === 1}
